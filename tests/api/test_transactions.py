...
def test_should_not_make_transaction_without_api_key(client: TestClient) -> None:
    ...
    response = client.post(
        "/transactions",
        headers={"api_key": unknown_api_key},
        json={
            "from_address": str(uuid4()),
            "to_address": str(uuid4()),
            "transaction_amount": 0.5,
        },
    )
...
def test_make_transaction_unknown_wallet(client: TestClient) -> None:
    ...
    response = client.post(
        ...
        headers={"api_key": api_key},
        json={
            "from_address": unknown_wallet_address,
            "to_address": str(uuid4()),
            "transaction_amount": 0.5,
        },
    )
    ...
    assert response.json() == {
        "error": {
            "message": f"Wallet with address<{unknown_wallet_address}> does not exist."
        }
    }
...
def test_make_transaction_same_wallet(client: TestClient) -> None:
    ...
    response = client.post(
        ...
        json={
            "from_address": unknown_wallet_address,
            "to_address": unknown_wallet_address,
            "transaction_amount": 0.5,
        },
    )
...
def test_make_transaction_with_other_wallet(client: TestClient) -> None:
    ...
    response = client.post(
        ...
        json={
            "from_address": wallet_address2,
            "to_address": wallet_address1,
            "transaction_amount": 0.5,
        },
    )
